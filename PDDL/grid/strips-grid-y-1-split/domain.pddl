...
(:action pickup-new-key
:parameters (?curpos ?newkey)
:precondition (and (place ?curpos) (key ?newkey) (at-robot ?curpos) (at ?newkey ?curpos) (arm-empty))
:effect (and (holding ?newkey) (not (at ?newkey ?curpos)) (not (arm-empty))))

(:action putdown-old-key
:parameters (?curpos ?oldkey)
:precondition (and (place ?curpos) (key ?oldkey) (at-robot ?curpos) (holding ?oldkey))
:effect (and (at ?oldkey ?curpos) (not (holding ?oldkey)) (arm-empty)))
