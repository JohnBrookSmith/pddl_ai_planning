(define
    ...
    (:predicates
        (engine_running) ; The engine is running
        (engine_stopped) ; The engine is not running
    )
    ...
    ; The speed limit constraint ensures that the velocity does not exceed the maximum speed
    (:constraint speed_limit
        :parameters ()
        ; The condition checks that the velocity is within the range of -max_speed to max_speed
